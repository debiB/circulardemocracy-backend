...
app.openapi(listPoliticiansRoute, async (c) => {
    const db = c.get("db");
    const data = await db.request("/politicians?select=id,name,email,party,country,region,position,active");
    return c.json(data);
});
...
app.openapi(getPoliticianRoute, async (c) => {
    ...
    const data = await db.request(`/politicians?id=eq.${id}&select=id,name,email,party,country,region,position,active&limit=1`);
