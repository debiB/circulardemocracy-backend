...
import { DatabaseClient, hashEmail } from "./database";
// =============================================================================
...
app.openapi(mtaHookRoute, async (c) => {
    ...
    try {
        ...
        const results = await Promise.all(hookData.recipients.map(async (recipientEmail) => {
            return await processEmailForRecipient(db, c.env.AI, hookData, senderEmail, senderName, recipientEmail);
        }));
        // Use the result with highest confidence
        const bestResult = results.reduce((best, current) => (current.confidence || 0) > (best.confidence || 0) ? current : best);
...
// =============================================================================
async function processEmailForRecipient(db, ai, hookData, senderEmail, _senderName, recipientEmail) {
    try {
