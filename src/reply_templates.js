...
app.openapi(listReplyTemplatesRoute, async (c) => {
    const db = c.get("db");
    const data = await db.request("/reply_templates?select=*");
    return c.json(data);
});
...
app.openapi(getReplyTemplateRoute, async (c) => {
    ...
    const data = await db.request(`/reply_templates?id=eq.${id}&select=*&limit=1`);
...
app.openapi(createReplyTemplateRoute, async (c) => {
    ...
    const data = await db.request("/reply_templates", {
        method: "POST",
