import { hashEmail, } from "./database";
...
export async function processMessage(db, ai, data) {
    // 1. Politician Lookup
    ...
    // 2. Duplicate Check
    const existingMessage = await db.getMessageByExternalId(data.external_id, data.channel_source || "unknown");
    if (existingMessage) {
