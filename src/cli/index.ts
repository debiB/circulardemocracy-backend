...
import { processMessage, PoliticianNotFoundError, type Ai, type MessageInput } from "../message_processor.js";
import { z } from "zod";

// Use the same validation schema as the REST API
...
// Mock AI interface for CLI - using Cloudflare Workers AI compatible interface
class MockAi implements Ai {
  async run(model: string, inputs: any): Promise<any> {
    // For CLI, we'll use a simple embedding generation or fallback
    // In a real deployment, this would connect to Cloudflare Workers AI
    throw new Error("AI service not available in CLI mode. Please set up AI embedding service.");
  }
}
...
export function parseArgs(): MessageInput | null {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    // Handle flags that might contain spaces in values
  ...
  // Set default channel source for CLI
  parsed.channel_source = parsed.channel_source || "cli";

  // Map CLI argument names to schema field names
...
function printUsage() {
  console.log(`
Circular Democracy CLI - Message Processing Tool

USAGE:
  cli --message-id <id> --recipient-email <email> --sender-name <name> \\
...
// Mock embedding generation for CLI (placeholder)
async function generateMockEmbedding(text: string): Promise<number[]> {
  // This is a placeholder - in production, you'd want to integrate with an actual embedding service
...
// Custom AI implementation for CLI that uses mock embeddings
...
async function main() {
  ...
  try {
    ...
    // Initialize AI (using CLI mock implementation)
    const ai: Ai = new CliAi();

    console.log('Processing message...');

    // Process the message using the existing pipeline
